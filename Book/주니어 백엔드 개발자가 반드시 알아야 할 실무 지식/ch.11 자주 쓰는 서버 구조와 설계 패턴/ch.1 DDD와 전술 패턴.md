...
## 5. Order-Payment 같은 별개 Aggregate 간 트랜잭션 처리

### 5.1 먼저 판단할 것: 정말 별개 Aggregate인가?

- 두 개가 항상 같이 롤백돼야 하는 강한 불변식으로 묶여 있다면 → 사실 하나의 Aggregate일 수 있음
- Payment가 재시도/부분취소/PG 콜백 등 자기만의 라이프사이클을 갖는다면 → 별개 Aggregate (실무 대부분 이 경우)

### 5.2 "UseCase에서 Payment를 저장해도 되는가"의 진짜 기준

문제는 저장 자체가 아니라 **"UseCase가 Payment의 도메인 규칙(생성 조건, 검증)까지 판단하고 있는가"**다. UseCase는 조율만 하고, 각 Aggregate가 자기 규칙은 스스로 지키게(`Payment.createFor(...)` 내부에서 자체 검증) 만들면 같은 트랜잭션에서 저장해도 무방하다.

### 5.3 케이스별 설계

|상황|설계|
|---|---|
|Payment 실패 시 Order도 반드시 롤백돼야 함 (강한 일관성 필수)|같은 트랜잭션에서 두 Aggregate 저장. UseCase는 조율만, 로직은 각 Aggregate 내부에|
|Payment 실패가 Order를 무효화하지 않음 (비동기 PG 콜백, 재시도 등)|Domain Event로 분리 → 별도 트랜잭션/별도 핸들러에서 Payment 생성|
|향후 Payment가 별도 서비스로 분리될 가능성 있음|처음부터 이벤트 기반으로 설계|

### 5.4 이벤트로 분리할 경우 주의점

- Order는 저장됐는데 이벤트 발행이 유실되면 Payment가 안 생성되는 위험이 존재한다.
- 이를 막으려면 **Outbox 패턴**(이벤트를 Order와 같은 트랜잭션에서 Outbox 테이블에 함께 기록 → 별도 프로세스가 읽어 안정적으로 발행)을 고려한다.

> **핵심**: "UseCase에서 Payment 저장 금지"가 아니라, **"UseCase가 판단 로직을 갖고 있는가"** + **"Order-Payment 간 강한 일관성이 정말 필요한가"**로 판단한다.